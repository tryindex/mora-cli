...
/**
 * How a reader gets Google credentials onto their machine. Named here because
 * every BigQuery failure that is really an auth failure should say it.
 */
...
export async function chooseSettings(
  ...
  // Said before the prompts rather than after a failure: a BigQuery connection
  // with no key file authenticates as whoever gcloud last signed in, and that is
  // the one setting the prompts cannot ask about.
  if (type === 'bigquery' && interactive) {
    prompts.log.info(
      `Leave the key file empty to use the credentials \`${ADC_LOGIN_COMMAND}\` wrote.`,
    );
  }
  ...
  for (const setting of connectionSettings(type, context)) {
    ...
    const answer = unlessCancelled(
      await prompts.text({
        message: settingMessage(setting),
...
function settingMessage(setting: ConnectionSetting): string {
  return setting.required ? setting.label : `${setting.label} ${pc.dim('(optional)')}`;
}
...
function addNextSteps(context: {
  ...
  if (missingEnvVars.length > 0) {
    ...
    steps.push(`Run \`mora schema --connection ${connectionName}\` to see what it can read.`);
    steps.push(
      `Add a source to a model: \`source: my_table is ${connectionName}.table('dataset.my_table')\`, then run \`mora validate\`.`,
    );
  }
